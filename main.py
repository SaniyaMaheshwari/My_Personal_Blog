...
##CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///new-blog.db'
...
def contact():
    message = "Contact Me"
    return render_template("contact.html", current_user = current_user, message = message)
...
def recieve_data():
    ...
    message_to_send = f"Subject: New Message\n\nName: {name}\nEmail: {email}\nPhone: {phone}\nMessage: {message}" 

    smtp.sendmail(os.environ.get('EMAIL'), os.environ.get('RECIEVER'), message_to_send) 
    smtp.quit()
    return render_template("contact.html", current_user = current_user, message = message)
